...
# 5 what is the loan_id of the highest payment recieved in the loan table?
Select loan_id
from loan
...
# 7What are the account_ids with the lowest loan amount that have a loan duration of 60 in the loan table?
...
## 10 for orders between order_id 29540 and order_id 29560 (inclusive)?
...
## 17In the loan table, for each day in December 1997, count the number of loans issued for each unique loan duration, ordered by date and duration, both in ascending order. You can ignore days without any loans in your output.
select count(*), date, duration as unique_loan
from loan
...
SELECT 
    ...
    SUM(CASE WHEN type = 'VYDJ' THEN amount ELSE 0 END) AS outgoing_amount,
    SUM(CASE WHEN type = 'PRIJEM' THEN amount ELSE 0 END) - 
    SUM(CASE WHEN type = 'VYDJ' THEN amount ELSE 0 END) AS difference
FROM 
    trans
WHERE 
    account_id = 396;
